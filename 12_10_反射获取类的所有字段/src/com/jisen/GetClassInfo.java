...
//				System.out.print(" "+MySplitStr(co.toString()));//利用字符截取把截取出来，但是有一个小问题
	...
	private static void printAllFields(Class<?> c) {
		Field[] fs=c.getDeclaredFields();//获取成员变量放到一个数组上
		if(fs.length !=0){//如果这个数组不为空
			for(Field ft:fs){
				//获取修饰符,返回值是一个int值
				int iMod=ft.getModifiers();
				if(iMod!=0){//如果有修饰符
					System.out.print("\t"+Modifier.toString(ft.getModifiers())+" ");
				}else{
					System.out.print("\t");
				}
				//获取数据类型
				System.out.print(MySplitStr(ft.getType().getName())+" ");
				//获取变量名
...
//			Constructor<?> con =c8.getDeclaredConstructor();//调用一个无参构造器，出现访问权限异常那么就要开放无参构造器的权限
		...
		//修改成员变量的值
		try {
			Field f=c8.getDeclaredField("name");
			f.setAccessible(true);//将修饰符权限开放
			f.set(obj, "王小锤");//直接操作私有成员
			Object o=f.get(obj);
